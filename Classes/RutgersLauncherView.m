...
static const CGFloat kRutgersLauncherViewMargin = 0;
static const CGFloat kRutgersLauncherViewPadding = 0;
static const CGFloat kRutgersLauncherViewPagerHeight = 20;
static const CGFloat kRutgersLauncherViewWobbleRadians = 1.5;
static const CGFloat kRutgersLauncherViewSpringLoadFraction = 0.18;
static const NSTimeInterval kRutgersLauncherViewEditHoldTimeInterval = 1;
static const NSTimeInterval kRutgersLauncherViewSpringLoadTimeInterval = 0.5;
static const NSTimeInterval kRutgersLauncherViewWobbleTime = 0.07;
static const NSInteger kRutgersLauncherViewDefaultColumnCount = 3;
static const NSTimeInterval kRutgersLauncherViewTransitionDuration = 0.3;
static const NSTimeInterval kRutgersLauncherViewFastTransitionDuration = 0.2;


@interface RutgersLauncherScrollView : UIScrollView
@end

@implementation RutgersLauncherScrollView
...
@implementation RutgersLauncherView

@synthesize columnCount, rowCount, delegate, editable, lastIndex;
...
- (id)initWithFrame:(CGRect)frame withRowCount:(int)newRowCount withColumnCount:(int)newColumnCount
{
    if ((self = [super initWithFrame:frame])) 
	{
		...
		scrollView = [[RutgersLauncherScrollView alloc] initWithFrame:CGRectMake(0, 0, frame.size.width, frame.size.height - kRutgersLauncherViewPagerHeight - 30)];
...
-(RutgersLauncherButton*)addButtonWithItem:(RutgersLauncherItem*)item
{
	RutgersLauncherButton *button = [[RutgersLauncherButton alloc] initWithFrame:CGRectZero];
...
- (NSMutableArray*)pageWithItem:(RutgersLauncherItem*)item 
{
...
- (NSIndexPath*)indexPathOfItem:(RutgersLauncherItem*)item 
{
...
- (NSMutableArray*)pageWithButton:(RutgersLauncherButton*)button 
{
...
- (void)layoutButtons 
{
	[self layoutIfNeeded];
	 ...
	 CGFloat buttonWidth = ceil((self.frame.size.width - (kRutgersLauncherViewMargin*2 + kRutgersLauncherViewPadding*(self.columnCount-1))) / self.columnCount);
	 CGFloat buttonHeight = [self rowHeight];
	 CGFloat pageWidth = scrollView.frame.size.width;
	 
	 CGFloat x = kRutgersLauncherViewMargin, minX = 0;
	 for (NSMutableArray* buttonPage in buttons) 
	 {
		 CGFloat y = kRutgersLauncherViewMargin;
		 for (RutgersLauncherButton* button in buttonPage) 
		 {
			 ...
			 x += buttonWidth + kRutgersLauncherViewPadding;
			 if (x >= minX+pageWidth) 
			 {
				 y += buttonHeight + kRutgersLauncherViewPadding;
				 x = minX+kRutgersLauncherViewMargin;
			 }
		 }
...
- (void)recreateButtons 
{
	...
	for (NSArray* page in pages) 
	{
		...
		for (RutgersLauncherItem* item in page) 
		{
			RutgersLauncherButton* button = [self addButtonWithItem:item];
...
- (void)scrollToItem:(RutgersLauncherItem*)item animated:(BOOL)animated 
{
...
- (void)layoutSubviews 
{
	...
	pager.frame = CGRectMake(0, scrollView.frame.size.height, self.frame.size.width, kRutgersLauncherViewPagerHeight);
...
- (void)startDraggingButton:(RutgersLauncherButton*)button withEvent:(UIEvent*)event 
{
	...
	[UIView setAnimationDuration:kRutgersLauncherViewFastTransitionDuration];
...
- (void)deselectButton:(RutgersLauncherButton*)button 
{
	[button setSelected:NO];
}

- (void)buttonTouchedUpInside:(RutgersLauncherButton*)button 
{
	if (editing) 
	{
	...
	else 
	{
		...
		[self performSelector:@selector(deselectButton:) withObject:button afterDelay:kRutgersLauncherViewTransitionDuration];
...
- (void)buttonTouchedUpOutside:(RutgersLauncherButton*)button 
{
	if (editing) 
	{
		if (button == dragButton) 
...
- (void)wobble 
{
	...
	if (editing) 
	{
		CGFloat rotation = (kRutgersLauncherViewWobbleRadians * M_PI) / 180.0;
		...
		if (nWobblyButtons >= 1) 
		{
			[UIView setAnimationDuration:kRutgersLauncherViewWobbleTime];
			[UIView setAnimationDelegate:self];
		...
		else 
		{
			...
			[self performSelector:@selector(wobble) withObject:nil afterDelay:kRutgersLauncherViewWobbleTime];
		}
...
- (RutgersLauncherButton*)buttonForItem:(RutgersLauncherItem*)item 
{
	NSIndexPath* path = [self indexPathOfItem:item];
	if (path) 
	{
...
- (void)removeItem:(RutgersLauncherItem*)item animated:(BOOL)animated 
{
	...
	if (itemPage) 
	{
		RutgersLauncherButton* button = [self buttonForItem:item];
...
- (void)closeButtonTouchedUpInside:(RutgersLauncherButton*)closeButton 
{
	for (NSArray* buttonPage in buttons) 
	{
		for (RutgersLauncherButton* button in buttonPage) 
...
- (void)endEditing 
{
	...
	[UIView beginAnimations:nil context:nil];
	[UIView setAnimationDuration:kRutgersLauncherViewTransitionDuration];
	[UIView setAnimationDelegate:self];
...
- (void)editHoldTimer:(NSTimer*)timer 
{
	...
	RutgersLauncherButton *button = [data objectAtIndex:0];
...
- (void)buttonTouchedDown:(RutgersLauncherButton*)button withEvent:(UIEvent*)event 
{
	if (editing) 
	{
		if (!dragButton) 
	...
	else 
	{
		TI_INVALIDATE_TIMER(editHoldTimer);
        if (editable) {
            editHoldTimer = [NSTimer scheduledTimerWithTimeInterval:kRutgersLauncherViewEditHoldTimeInterval
...
- (void)updateTouch 
{
	...
	CGFloat springLoadDistance = dragButton.frame.size.width*kRutgersLauncherViewSpringLoadFraction;
	...
	if (goToPreviousPage || goToNextPage) 
	{
		if (!springLoadTimer) 
		{
			springLoadTimer = [NSTimer scheduledTimerWithTimeInterval:kRutgersLauncherViewSpringLoadTimeInterval
